...
function GoogleIcon() {
  return (
    <svg className={styles.providerIcon} viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
...
export function LoginPage() {
  ...
  if (!sessionReady) {
    return (
      <div className={`${authPage.page} ${styles.loginPage}`}>
  ...
  return (
    <div className={`${authPage.page} ${styles.loginPage}`}>
      <div className={styles.shell}>
        <h1 className={styles.title}>{APP_NAME} Login</h1>
        ...
        {providerCount > 0 ? (
          <div className={styles.providers}>
            {providers?.google ? (
              <button type="button" className={`${styles.provider} ${styles.providerGoogle}`} onClick={startGoogle}>
                <GoogleIcon />
            ...
            {providers?.github ? (
              <button type="button" className={`${styles.provider} ${styles.providerGithub}`} onClick={startGithub}>
                <GitHubIcon />
            ...
            {providers?.guest ? (
              <button
                type="button"
                className={`${styles.provider} ${styles.providerGuest}`}
        ...
        {!providers?.google && !providers?.github && !providers?.guest && !providers?.dev && providers !== null ? (
          <p className={styles.empty}>No sign-in providers are configured.</p>
        ) : null}
