  ...
  static async loadTextures(idleSrc: string, walkSrc: string): Promise<WalkTextureSet | null> {
    try {
      ...
      return {
        ...
        idleLeft: Entity.sliceSpritesheetRow(idleBase, SHEET_ROW_LEFT, FRAMES_PER_ROW, WALK_ENTITY_SPRITE_SIZE_PX),
        idleDown: Entity.sliceSpritesheetRow(idleBase, SHEET_ROW_DOWN, FRAMES_PER_ROW, WALK_ENTITY_SPRITE_SIZE_PX),
        idleUp: Entity.sliceSpritesheetRow(idleBase, SHEET_ROW_UP, FRAMES_PER_ROW, WALK_ENTITY_SPRITE_SIZE_PX),
      };
    } catch {
      return null;
    }
