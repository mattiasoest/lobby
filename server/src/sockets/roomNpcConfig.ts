...
/** Fixed UUIDs shared with the client (`client/src/game/config/roomNpc.ts`). */
export const NPC_USER_IDS = {
...
export const NPC_RATE_LIMIT_FALLBACK_MESSAGES = [
  'Go away! Im busy',
...
export function pickRandomNpcFallbackMessage(): string {
  const pool = NPC_RATE_LIMIT_FALLBACK_MESSAGES;
  return pool[Math.floor(Math.random() * pool.length)] ?? pool[0];
}

const ROOM_NPC_CONFIGS: Record<RoomId, Omit<RoomNpcConfig, 'roomId'>> = {
  1: {
    userId: NPC_USER_IDS[1],
    username: 'Grunk',
...
export function getRoomNpcConfig(roomId: number): RoomNpcConfig | null {
  const key = roomId as RoomId;
  if (!(key in ROOM_NPC_CONFIGS)) return null;
  return { roomId, ...ROOM_NPC_CONFIGS[key] };
}
...
export function messageMentionsNpc(content: string, npcUsername: string): boolean {
  const npcKey = usernameForMentionMatch(npcUsername);
  for (const match of content.matchAll(MENTION_RE)) {
    const bare = (match[1] ?? '').toLowerCase();
    if (bare === npcKey) return true;
  }
  return false;
}

/** True when content includes text beyond @mention(s) of this NPC. */
export function npcMentionIncludesMessage(content: string, npcUsername: string): boolean {
  const npcKey = usernameForMentionMatch(npcUsername);
  const withoutNpcMentions = content.replace(MENTION_RE, (full, username: string) => {
    const bare = username.toLowerCase();
    return bare === npcKey ? '' : full;
  });
